...
class P4Adapter:
    ...
    def _gen_p4(self, sw, pipe):
        ...
        variables = set()
        for tid, table in sorted(pipe.items()): # type: int, PipelineTable
            matches, regs, action_parm_str, action_str = self._parse_table(table)
    ...
    def _parse_table(self, table):
        ...
        for field in table.matches:
            if field=='pkt.eth.src':
                matches.append({'name': 'hdr.ethernet.src', 'type': TERNARY})
            elif field == 'pkt.eth.dst':
                matches.append({'name': 'hdr.ethernet.dst', 'type': TERNARY})
            elif field == 'pkt.eth.dst':
                matches.append({'name': 'hdr.ethernet.dst', 'type': TERNARY})
            elif field == 'inport_label' or field=='inport':
                matches.append({'name': 'standard_metadata.ingress_port', 'type': TERNARY})
            elif field == 'vlan':
                matches.append({'name': 'hdr.tag.tag', 'type': TERNARY})
            elif field =='pkt':
                ...
                if reg:
                    regs.append(reg)
                    matches.append({'name': "meta."+reg, 'type': TERNARY})
                else:
        ...
        if table.flowRules: # type: FlowRule
            ...
            if len(actions)==1:
                a = actions[0]
                if a[0]=='output':
                    action_parm_str = 'bit<9> port'
                    action_str = 'output(port);'
                elif a[0]=='set_field':
                    action_parm_str = 'bit<2> v'
                    action_str = 'meta.%s = v;'%a[2]
                elif a[0]=='controller':
                    action_str = 'controller();'
                else:
                ...
                if a1[0]=='pop_vlan' and a2[0]=='output':
                    action_parm_str = 'bit<9> port'
                    action_str = 'pop_tag_output(port);'
                elif a1[0]=='set_vlan' and a2[0]=='output':
                    action_parm_str = 'bit<16> tag, bit<9> port'
                    action_str = 'push_tag_output(tag, port);'
                else:
                    error('unhandled actions', actions)
            else:
                error('unhandled actions', actions)

        return matches, regs, action_parm_str, action_str
