...
class SwitchConnection(object):
    ...
    def start(self, i, q):
        _thread.start_new_thread(self.msg_thread, (i, q))

    def msg_thread(self, i, q):
        for msg in self.stream_msg_resp:
            if msg.packet:
                # print(msg.packet.payload)
                q.put((i, msg))
    ...
    def WriteTableEntryList(self, table_entry_list, dry_run=False):
        self.__tmp = table_entry_list
        request = p4runtime_pb2.WriteRequest()
        request.device_id = self.device_id
        request.election_id.low = 1
    ...
    def DeleteAllTableEntry(self):
        ...
        #     update.entity.table_entry.CopyFrom(e)
        # try:
        #     self.client_stub.Write(request)
        #     #import time
        #     #time.sleep(0.1)
        ...
        #     #os._exit(-1)
        # self.__tmp = []
        tids = set(t.table_id for t in self.__tmp)
        for tid in tids:
